...
- Versnellingsonderzoek 28-08-2026 op de 112 MB-export (rapport en meetstraat in
  ~/gwsw-orox-helpers-onderzoek/2026-08-28-50procent/: RAPPORT.md, meet.sh, gelijk.py,
...
- Export: ~/nlriochecker/data/gwsw_orox_ttl/dewoldenhoogeveen_orox.ttl (112 MB, cp850-bytes in
...
const DIMENSIONS = [
  { key: 'arch-afnemer', prompt: `${ARCH_SKILLS}\nLENS: ARCHITECTUUR — het interface gezien vanuit de afnemer. De enige afnemer staat lokaal: ~/nlriochecker/src/nlriochecker/ (grep daar op 'from gwsw_orox_helpers' — 20+ importplekken, vooral dataset: Conduit, Node, GwswDataset, part_holders_of, aspect_holders_of, parts_of, HAS_CONNECTION, GWSW, Inwinning, markeer_vulwaarden; cache: laad_met_cache, CacheUitslag; voortgang). Lees die afnemerbestanden (een keer volledig) en beoordeel de diepte van het interface: hoeveel moet nlriochecker weten (IRI-constanten, RdfNode, losse module-functies naast methoden) om een check te schrijven, welke samenstellingen herhaalt de afnemer zelf die de bibliotheek had moeten dragen (patronen die 3x+ terugkomen), en welke leaks van implementatie (rdflib-termen, pyoxigraph, pickle) bereiken de afnemer? Tel het oppervlak dat de afnemer nu leert en schets een dieper interface dat het halveert — additief naast het bevroren oppervlak (raaktContract=true waar het pin raakt). Bewijs: bestand:regel in beide repo's en de telling.` },
