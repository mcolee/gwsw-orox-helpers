...
class GwswDataset:
    ...
    def subset(self, uris: Iterable[str]) -> GwswDataset:
        ...
        `geometry_errors` snijdt wel mee, en sinds issue #36 treft dat filter ook echt
        iets: de meldingen staan op de knoop of de streng en niet meer op de orientatie
        eronder, waardoor sleutels en `behouden` niet langer disjunct zijn.
        `inlezen._geometry` draagt het waarom. De enige sleutel die geen object is, is
        die van een orientatie zonder houder; zo'n wees valt hier weg, en terecht, want
        hij is geen knoop en geen streng.
        """
