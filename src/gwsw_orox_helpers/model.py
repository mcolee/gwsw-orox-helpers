"""Het domeinmodel van een ingelezen GWSW-OroX-dataset: `GwswDataset` en `GwswVersie`.

Dit is de bak achter het gezicht. `dataset` her-exporteert wat hier staat; wie de
dataset-vragen zelf wil lezen of wijzigen, leest deze module. De lader (`laden`) bouwt een
`GwswDataset` op en de transformatie (`vulwaarden`) leidt er een nieuwe uit af; allebei
importeren zij deze module en niet andersom -- het model weet van de lader niets, zodat de
importrichting `dataset -> laden -> model` een lijn blijft en geen cyclus (issue #67).

De leeslaag eronder is in zes modules verdeeld, elk met een eigen vraag:
...
Elke naam die een afnemer uit `gwsw_orox_helpers.dataset` importeert, komt hier ook naar
buiten -- de klassen, de IRI-constanten en de graafhulpen -- met dezelfde handtekening en
hetzelfde gedrag (Harde regel in `CLAUDE.md`; `tests/test_publieke_api.py` is de
scheidsrechter). `dataset` her-exporteert `__all__` van deze module ongewijzigd.
"""
...
# De lijst is het oppervlak dat `dataset` her-exporteert (issue #67), niet een keuze van
# deze module: alles wat ooit uit `gwsw_orox_helpers.dataset` te importeren was en niet in
# de lader (`laden`) of de transformatie (`vulwaarden`) thuishoort, staat erin -- ook de
# namen die na een hersnit in `geometry` (`parse_gml`, `parse_gml_z`, `is_multipart_literal`,
# `GeometryError`) of in `domein` (`ISO_DATUM`, `JAARTAL`) terechtkwamen. Ze zijn hier niets
# meer dan een doorgeefluik; wie ze uit hun eigen module haalt, krijgt hetzelfde object. Bij
# twijfel over "is dit publiek?" blijft de naam staan: hem weghalen breekt een afnemer stil,
# hem laten staan kost niets.
__all__ = [
    "FANTOOM_STAART",
    "GWSW",
    "HAS_ASPECT",
...
class GwswDataset:
    ...
    #
    # Bewust een `list`-memo en geen `functools.cached_property` (afweging bij issue #67): een
    # `cached_property` is geen `property`, en de publieke leesweg `gwsw_versie` staat in
    # `tests/test_publieke_api.py` gepind als `property` (`isinstance(..., property)`). De
    # list-memo houdt de leesweg een echte `property` en blijft daarmee binnen dat contract.
